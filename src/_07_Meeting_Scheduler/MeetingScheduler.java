...
public class MeetingScheduler {
	...
	public static Schedule getMutualAvailability(Schedule person1, Schedule person2) {
		...
		for (String temp : person1.getSchedule().keySet()) {
			arraylist = person2.getSchedule().get(temp);	
			sched.getSchedule().put(temp, new ArrayList<Integer>());
			for (int i = 0; i < arraylist.size(); i++) {
				arraylist.get(i);
				if (person1.getSchedule().get(temp).contains(arraylist.get(i))) {
					sched.getSchedule().get(temp).add(arraylist.get(i));
