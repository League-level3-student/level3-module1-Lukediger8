...
public class RetroSun extends PApplet {
	...
	int sunTopY = HEIGHT / 2 - 200;
	int sunBottomY = HEIGHT / 2 + 200;
	float step;
	static final int NUM_SECTIONS = 6;
	Rectangle[] section;
	// RGB colors
	...
	public void setup() {
		...
		background(bgColor);
		section = new Rectangle[NUM_SECTIONS];
		for (int i = 0; i < NUM_SECTIONS; i++) {
			section[i] = new Rectangle(0, 600 - i * 50, 800, 30);
		}
	}
	...
	public void draw() {
		...
		for (int i = 0; i < pixels.length; i++) {
			if (pixels[i] == sunColors[0]) {
				...
				jay = interpolateColor(sunColors, step);
				pixels[i] = jay;
		...
		for (int i = 0; i < section.length; i++) {
			...
			section[i].y -= 2;
			section[i].h = section[i].y / 5 -40;
			section[i].x = WIDTH / 2 - 200;
			section[i].w = 2 * 200;
			if (section[i].y < 250 ) {
				section[i].y = 550;
			}

			rect(section[i].x, section[i].y, section[i].w, section[i].h);
